 ...
 *   @param  {String} [maxImageWidth] The maximum width of the image
...
function htmlToPdfMake(htmlText, options) {
	...
	this.removeTagClasses =
		options && typeof options.removeTagClasses === 'boolean'
			...
			: false;
	this.ignoreStyles =
		options && Array.isArray(options.ignoreStyles) ? options.ignoreStyles : [];
	this.id = options && options.id ? options.id : 0;
	this.proxy = options && options.proxy ? options.proxy : null;
	this.maxImageWidth =
		options && options.maxImageWidth ? options.maxImageWidth : null;
	...
	this.convertHtml = function (htmlText) {
		...
		if (this.removeExtraBlanks)
			htmlText = htmlText
				.replace(
					/(<\/?(div|p|h1|h2|h3|h4|h5|h6|ul|li)([^>]+)?>)\s+(<\/?(div|p|h1|h2|h3|h4|h5|h6|ul|li))/gi,
					'$1$4'
				)
				.replace(
					/(<\/?(div|p|h1|h2|h3|h4|h5|h6|ul|li)([^>]+)?>)\s+(<\/?(div|p|h1|h2|h3|h4|h5|h6|ul|li))/gi,
		...
		switch (element.nodeType) {
			...
			case 1: {
				...
				switch (nodeName) {
                    ...
                    if (this.width > _this.maxImageWidth) {
                      var ratio = _this.maxImageWidth / this.width;
                      ret.width = _this.maxImageWidth;
                      ret.height = this.height * ratio;
                    }
                    ...
                    _this.applyStyle({ ret: ret });
										reject('Error loading image');
									};
									img.src = src;
								});

								this.imagesRef.push(promise);
							}
						} else {
	...
	this.applyStyle = function (params) {
		...
		if (!params.parents || params.parents.length === 0) return params.ret;
	...
	this.parseStyle = function (element, ignoreProperties) {
		...
		if (width) {
			style.unshift(
				'width:' + this.convertToUnit(width + (isNaN(width) ? '' : 'px'))
			);
		}
		if (height) {
			style.unshift(
				'height:' + this.convertToUnit(height + (isNaN(height) ? '' : 'px'))
			);
		}
	...
	return new Promise(function (resolve, reject) {
		var result = _this.convertHtml(htmlText);
		// if we only pass a string without HTML code
		if (typeof result === 'string') result = { text: result };
		// if images by reference
		if (_this.imagesByReference) {
			result = { content: result, images: {} };
			var id = _this.id;
			var wrappedPromises = _this.imagesRef.map(function (p) {
				return Promise.resolve(p).then(
					function (value) {
						var fulfilledObj = { status: 'fulfilled' };
						for (var key in value) {
							fulfilledObj[key] = value[key];
						}
						return fulfilledObj;
					},
					function (error) {
			...
			Promise.all(wrappedPromises).then(function (results) {
				results.forEach(function (_result, i) {

          if (_result.status === 'fulfilled') {
            var src = _result.src;
            // check if 'src' is a JSON string
            result.images['img_ref_' + id + '_' + i] = src.startsWith('{')
              ? JSON.parse(src)
              : src;
          };
